...
namespace ATC {
class ZAxisConfiguratorServiceTest : public Test {
public:
...
TEST_F(
    ZAxisConfiguratorServiceTest,
    GIVEN_zAxisConfiguratorService_WHEN_saveStartPositionIsCalled_THEN_persistentConfigIsSaved
) {
    ...
    zAxisConfiguratorService_.resetBufferedConfig();

    EXPECT_CALL(
        persistentStorageControllerMock_, saveZAxisConfig(config)
    );

    zAxisConfiguratorService_.saveStartPosition();
}
}